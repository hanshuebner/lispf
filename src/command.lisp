...
Returns a list of (name doc &optional aliases) entries."
  (let ((entries nil)
    ...
    ;; Screens reachable from menus (always fully loaded, unlike screen cache)
    (maphash (lambda (menu-name menu-data)
               (let ((menu-title (getf menu-data :title)))
                 (dolist (item (getf menu-data :items))
                   (let* ((screen-sym (getf item :screen))
                          (name (string-downcase (symbol-name screen-sym))))
                     (unless (gethash name seen)
                       (setf (gethash name seen) t)
                       (let* ((label (getf item :label))
                              (desc (getf item :description))
                              (doc (if (string-equal menu-name "main")
                                       (format nil "~A - ~A" label desc)
                                       (format nil "~A / ~A - ~A"
                                               menu-title label desc)))
                              (info (gethash name (application-screens app)))
                              (aliases (when info (screen-info-aliases info))))
                         (push (list name doc aliases) entries)
                         (dolist (alias aliases)
