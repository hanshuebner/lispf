...
(define-test parse-prefix-case ()
  (multiple-value-bind (cmd count) (ed:parse-prefix-command "UC")
    (assert-equal :uc cmd)
    (assert-equal 1 count))
  (multiple-value-bind (cmd count) (ed:parse-prefix-command "LC3")
    (assert-equal :lc cmd)
    (assert-equal 3 count)))
...
(define-test parse-prefix-overtyped-line-number ()
  ;; When user types "DD" over "000001", the field contains "DD0001"
  (multiple-value-bind (cmd count) (ed:parse-prefix-command "DD0001")
    (assert-equal :dd cmd "DD overtyped on line number should be block delete"))
  ;; Similarly for other block commands
  (multiple-value-bind (cmd) (ed:parse-prefix-command "CC0001")
    (assert-equal :cc cmd "CC overtyped"))
  (multiple-value-bind (cmd) (ed:parse-prefix-command "MM0001")
    (assert-equal :mm cmd "MM overtyped"))
  (multiple-value-bind (cmd) (ed:parse-prefix-command "RR0001")
    (assert-equal :rr cmd "RR overtyped"))
  ;; Single commands with trailing digits from line number
  (multiple-value-bind (cmd count) (ed:parse-prefix-command "I 0001")
    (assert-equal :i cmd "I overtyped")
    (assert-equal 1 count "I with no explicit count"))
  (multiple-value-bind (cmd count) (ed:parse-prefix-command "I50001")
    (assert-equal :i cmd "I5 overtyped")
    (assert-equal 5 count "I5 should get count 5"))
  (multiple-value-bind (cmd count) (ed:parse-prefix-command "D 0001")
    (assert-equal :d cmd "D overtyped")
    (assert-equal 1 count "D with no explicit count"))
  (multiple-value-bind (cmd count) (ed:parse-prefix-command "D30001")
    (assert-equal :d cmd "D3 overtyped")
    (assert-equal 3 count))
  ;; A and B with trailing digits
  (multiple-value-bind (cmd) (ed:parse-prefix-command "A 0001")
    (assert-equal :a cmd "A overtyped"))
  (multiple-value-bind (cmd) (ed:parse-prefix-command "B 0001")
    (assert-equal :b cmd "B overtyped"))
  ;; UC/LC overtyped
